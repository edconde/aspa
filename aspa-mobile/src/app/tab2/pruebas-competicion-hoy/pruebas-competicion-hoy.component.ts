...
import { PruebaHoyModalPage } from './prueba-hoy-modal/prueba-hoy-modal.page';
...
@Component({
  selector: 'app-pruebas-competicion-hoy',
  templateUrl: './pruebas-competicion-hoy.component.html',
  styleUrls: ['./pruebas-competicion-hoy.component.scss'],
})
export class PruebasCompeticionHoyComponent implements OnInit {
  ...
  public chooseAction(p: Prueba): void {
    if (p.finalizada) {
      this.router.navigate(
        [`/fga/directo/${this.competicion.idcompeticion}/clasificaciones`],
        { queryParams: { idprueba: p.idprueba } }
      );
    } else if (new Date(p.fecha) < new Date()) {
      this.router.navigate(
        [`/fga/directo/${this.competicion.idcompeticion}/en-curso`],
        { queryParams: { idprueba: p.idprueba } }
      );
    } else {
